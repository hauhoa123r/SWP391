...
public class AppointmentRepositoryCustomImpl implements AppointmentRepositoryCustom {
    ...
    private String joinSearchAppointment(AppointmentDAI appointmentDAI){
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("JOIN staffs s ON a.doctor_id = s.staff_id\n");
        stringBuilder.append("JOIN hospitals h ON s.hospital_id = h.hospital_id\n");
        stringBuilder.append("JOIN departments d ON s.department_id = d.department_id\n");
        stringBuilder.append("JOIN services se ON a.service_id = se.service_id\n");
        return stringBuilder.toString();
    }
