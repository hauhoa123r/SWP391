...
public interface StaffRepository extends JpaRepository<Staff, Long>, JpaSpecificationExecutor<Staff> {
    ...
    @Query(value = "SELECT * FROM staffs WHERE staff_id = :id", nativeQuery = true)
    Optional<Staff> findByIdIncludingDeleted(@Param("id") Long id);

    @Query(value = "SELECT * FROM staffs WHERE deleted = true", nativeQuery = true)
    Page<Staff> findAllDeleted(Pageable pageable);
}
